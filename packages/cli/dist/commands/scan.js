...
export async function scanCommand(options) {
    ...
    const targetPath = options.target || process.env._4NDH4K_SKILLS_PATH;
    // Validate target path
    if (!targetPath) {
        console.error('Target path not set. Set _4NDH4K_SKILLS_PATH environment variable or use --target option.');
        process.exit(1);
    }
    // Scan for skills
